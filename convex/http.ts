...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    try {
      ...
      let workoutPlan = JSON.parse(workoutPlanText);// though i have given instruction not to give nay data out of jason format but still its ai so we need to parse the data obtained before sending to user
